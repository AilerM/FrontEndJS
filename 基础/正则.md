...
- match 查找匹配的String方法，它返回一个数组或者在未匹配到时返回null。
```js
...
- replace 执行查找匹配的String方法，并且使用替换字符串替换掉匹配到的子字符串。
```js
let str = 'ABc, bcd, fas'
str.replace(/a/ig,'*') // '*Bc,bcd,f*s'
```

- split 一个使用正则表达式或者一个固定字符串分隔一个字符串，并将分隔后的子字符串存储到数组中的String方法。
```js
let str = 'AAAAA##BBBBB##CCCC##DDD'
let newStr = str.split(/##/g).map(item=>{
  return `<p>${item}</p>`
...
```
> 分享
[正则校验](https://regexper.com/) 
[正则常用](https://www.jianshu.com/p/583998f435d0) 
